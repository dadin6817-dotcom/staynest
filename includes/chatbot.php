<?php
// includes/chatbot.php - Chatbot Component untuk Semua Halaman
?>
<!-- ========================================== -->
<!-- CHATBOT WIDGET - MUNCUL DI SEMUA HALAMAN -->
<!-- ========================================== -->
<div id="chatbotContainer" class="fixed bottom-24 right-6 z-[9999]">
    <!-- Tombol Chatbot -->
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (chatbotToggle) {
        chatbotToggle.addEventListener('click', function() {
            ...
            if (!chatbotWindow.classList.contains('hidden')) {
                if (chatbotIcon) chatbotIcon.className = 'fas fa-times text-white text-2xl';
                if (chatInput) chatInput.focus();
            } else {
                if (chatbotIcon) chatbotIcon.className = 'fas fa-robot text-white text-2xl';
            }
        });
    }
    ...
    function sendMessage(message) {
        ...
        userMessage.innerHTML = '<div class="bg-gradient-to-r from-purple-600 to-purple-800 text-white rounded-2xl rounded-tr-none p-3 shadow-sm max-w-[80%]"><p class="text-sm">' + escapeHtml(message) + '</p></div><div class="w-8 h-8 bg-gray-300 rounded-full flex items-center justify-center flex-shrink-0"><i class="fas fa-user text-gray-600 text-xs"></i></div>';
        ...
        typingIndicator.innerHTML = '<div class="w-8 h-8 bg-gradient-to-r from-purple-600 to-purple-800 rounded-full flex items-center justify-center flex-shrink-0"><i class="fas fa-robot text-white text-xs"></i></div><div class="bg-white rounded-2xl rounded-tl-none p-3 shadow-sm"><div class="flex gap-1"><span class="w-2 h-2 bg-purple-400 rounded-full animate-bounce"></span><span class="w-2 h-2 bg-purple-400 rounded-full animate-bounce" style="animation-delay: 0.1s"></span><span class="w-2 h-2 bg-purple-400 rounded-full animate-bounce" style="animation-delay: 0.2s"></span></div></div>';
        ...
        setTimeout(function() {
            ...
            botMessage.innerHTML = '<div class="w-8 h-8 bg-gradient-to-r from-purple-600 to-purple-800 rounded-full flex items-center justify-center flex-shrink-0"><i class="fas fa-robot text-white text-xs"></i></div><div class="bg-white rounded-2xl rounded-tl-none p-3 shadow-sm max-w-[80%]"><p class="text-sm text-gray-700">' + botResponse + '</p></div>';
    ...
    function getBotResponse(message) {
        ...
        if (msg.includes('booking') || msg.includes('pesan') || msg.includes('sewa')) {
            return '📝 Untuk booking:<br>1. Pilih properti di halaman <a href="/staynest/properties.php" class="text-purple-600 underline">Properties</a><br>2. Klik "View Details"<br>3. Isi tanggal check-in & check-out<br>4. Klik "Book Now"<br>5. Upload bukti pembayaran';
        }
        
        if (msg.includes('properti') || msg.includes('property') || msg.includes('lihat')) {
            return '🏠 Kami memiliki 3 properti unggulan:<br>• StayNest Vela - Babelan<br>• StayNest Aera - Tambun<br>• StayNest Elora - Babelan<br><br>Lihat di <a href="/staynest/properties.php" class="text-purple-600 underline">halaman Properties</a>';
        }
        
        if (msg.includes('harga') || msg.includes('price') || msg.includes('biaya')) {
            return '💰 Harga sewa mulai dari:<br>• Rp 700.000/bulan (Standard)<br>• Rp 800.000/bulan (VIP)<br><br>Sudah termasuk WiFi, Listrik, Air, dan Keamanan 24 jam!';
        }
        
        if (msg.includes('kontak') || msg.includes('admin') || msg.includes('hubungi')) {
            return '📞 Hubungi kami:<br>• Email: info@staynest.com<br>• WhatsApp: +62 812 3456 7890<br>• Instagram: @staynest.id';
        }
        
        if (msg.includes('bayar') || msg.includes('payment') || msg.includes('transfer')) {
            return '💳 Metode pembayaran:<br>• Transfer Bank: BCA, BRI, BNI, Mandiri, BSI<br>• Upload bukti transfer di halaman Payment<br>• Konfirmasi otomatis dalam 1x24 jam';
        }
        
        if (msg.includes('fasilitas') || msg.includes('kamar')) {
            return '🏠 Fasilitas kami:<br>✅ AC & WiFi<br>✅ Kamar mandi dalam<br>✅ Kasur & lemari<br>✅ Parkir luas<br>✅ CCTV 24 jam';
        }
        ...
        return 'Terima kasih atas pertanyaannya! 🙏<br><br>Untuk informasi lebih lanjut:<br>• Lihat <a href="/staynest/properties.php" class="text-purple-600 underline">halaman Properties</a><br>• Email: info@staynest.com<br>• WhatsApp: +62 812 3456 7890';
    }
